...
<div id="main-settings">
    <license-component verify-url="{{ route('settings.license.verify') }}"
        ...
        manage-license="{{ auth()->user()->hasPermission('core.manage.license') ? 'yes' : 'no' }}"></license-component>
</div>
{!! Form::open(['route' => ['settings.edit']]) !!}
<div class="max-width-1200">

    <div class="flexbox-annotated-section hidden">
        ...
        <div class="flexbox-annotated-section-content">
            <div class="wrapper-content pd-all-20">
                ...
                <div class="form-group mb-3" id="admin_email_wrapper" data-emails="{{ json_encode(get_admin_email()) }}"
                ...
                <div class="form-group mb-3">
                    ...
                    <div class="ui-select-wrapper">
                        <select name="locale" class="ui-select form-control select-search-full" id="locale">
                            ...
                            if (app()->environment('demo') && session('site-locale') &&
                            array_key_exists(session('site-locale'),
                            \Botble\Base\Supports\Language::getAvailableLocales())) {
                            $defaultLocale = session('site-locale');
                            }
                            @endphp
                            @foreach (\Botble\Base\Supports\Language::getAvailableLocales() as $key => $locale)
                            <option value="{{ $locale['locale'] }}" @selected($defaultLocale===$locale['locale'])>
                                {{ $locale['name'] }} - {{ $locale['locale'] }}</option>
                            @endforeach
                        </select>
                        <svg class="svg-next-icon svg-next-icon-size-16">
                            <use xmlns:xlink="http://www.w3.org/1999/xlink" xlink:href="#select-chevron"></use>
                ...
                <div class="form-group mb-3">
                    ...
                    <label class="me-2">
                        <input type="radio" name="locale_direction" value="ltr"
                            @checked(setting('locale_direction', 'ltr' )=='ltr'
                            )>{{ trans('core/setting::setting.locale_direction_ltr') }}
                    </label>
                    <label>
                        <input type="radio" name="locale_direction" value="rtl"
                            @checked(setting('locale_direction', 'ltr' )=='rtl'
                            )>{{ trans('core/setting::setting.locale_direction_rtl') }}
                    </label>
                </div>
                ...
                <div class="form-group mb-3">
                    ...
                    <label>
                        <input type="checkbox" value="1" @checked(setting('enable_send_error_reporting_via_email'))
            ...
            <div class="wrapper-content pd-all-20">
                ...
                <div class="form-group mb-3">
                    ...
                    <div class="admin-login-screen-backgrounds-setting">
                        {!! Form::mediaImages('login_screen_backgrounds[]', is_array(setting('login_screen_backgrounds',
                        '')) ? setting('login_screen_backgrounds', '') : json_decode(setting('login_screen_backgrounds',
                        ''), true)) !!}
                    </div>
                </div>
                ...
                <div class="form-group mb-3" style="display: none">
                    ...
                    <label class="me-2">
                        <input type="radio" name="admin_locale_direction" value="ltr"
                            @checked(setting('admin_locale_direction', 'ltr' )=='ltr'
                            )>{{ trans('core/setting::setting.locale_direction_ltr') }}
                    </label>
                    <label>
                        <input type="radio" name="admin_locale_direction" value="rtl"
                            @checked(setting('admin_locale_direction', 'ltr' )=='rtl'
                            )>{{ trans('core/setting::setting.locale_direction_rtl') }}
                    </label>
                </div>
                ...
                <div class="form-group mb-3">
                    ...
                    <label class="text-title-field"
                        for="rich_editor">{{ trans('core/setting::setting.general.rich_editor') }}</label>
                    ...
                    <label @class(['me-2'=> ! $loop->last])>
                        <input type="radio" name="rich_editor" value="{{ $key }}"
                            @checked(BaseHelper::getRichEditor()===$key)>
                        {{ $value }}
                    </label>
                    @endforeach
                </div>
                ...
                <div class="form-group mb-3" style="display: none">
                    ...
                    <div class="ui-select-wrapper">
                        <select name="default_admin_theme" class="ui-select" id="default_admin_theme">
                            ...
                            <option value="{{ $theme }}" @if (setting('default_admin_theme',
                                config('core.base.general.default-theme'))===$theme) selected @endif>
                                {{ Str::studly($theme) }}
                            </option>
                            @endforeach
                        </select>
                ...
                <div class="form-group mb-3" style="display: none">
                    ...
                    <label>
                        <input type="checkbox" value="1" @checked(setting('enable_change_admin_theme'))
